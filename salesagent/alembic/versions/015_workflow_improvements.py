...
def upgrade():
    ...
    try:
        op.create_table(
            ...
            sa.Column(
                "object_id", sa.String(100), nullable=False
        ...
        op.create_index(
            "idx_object_workflow_step", "object_workflow_mapping", ["step_id"]
        )
        op.create_index(
            "idx_object_workflow_created", "object_workflow_mapping", ["created_at"]
        )
    except:
...
def downgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["context_id"], ["contexts.context_id"], ondelete="SET NULL"
        ),
    )
    ...
    op.add_column(
        "workflow_steps", sa.Column("started_at", sa.DateTime(), nullable=True)
    )
